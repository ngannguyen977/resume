	...
	function init() {
		...
		rightSide.querySelector( 'div.intro-content' ).addEventListener( eventtype, function( ev ) {
			if(flagRight == 0) {
				...
				classie.add( splitlayout, 'intro-content-right' );
				if ($('html').hasClass('desktop')) {
		             new WOW().init();
		        } 
		        
				$('.slogan').fadeOut("fast");
				$("#test-circle, #test-circle2, #test-circle3, #test-circle4, #test-circle5").html('');
				$("#test-circle").circliful({
			            animation: 1,
			            animationStep: 5,
			            foregroundBorderWidth: 15,
			            backgroundBorderWidth: 15,
			            foregroundColor: '#1d4489',
			            percent: 90,
			            textSize: 28,
